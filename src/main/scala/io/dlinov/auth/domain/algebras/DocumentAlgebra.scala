...
class DocumentAlgebra[F[_]: Monad](
    dao: DocumentFDao[F],
    blobDao: BlobFDao[F],
    tmpBlobDao: BlobTmpFDao[F]
) extends BaseService {
  ...
  def createDocument(
    ...
    (for {
      ...
      doc ← EitherT {
        dao.create(customerId, documentType, documentTypeIdentifier, purpose, blobId, createdBy)
      }
    } yield doc).value
      .map(_.asServiceResponse)
  }

  def findById(id: UUID): F[ServiceResponse[Document]] = {
    for {
      doc ← dao.findById(id)
    } yield doc.asServiceResponse
  ...
  ): F[ServiceResponse[PaginatedResult[Document]]] = {
    for {
      docs ← dao.find(
  ...
  def approvePendingDocument(docId: UUID, approvedBy: String): F[ServiceResponse[Document]] = {
    (for {
      doc ← EitherT {
        dao
          ...
          .map(
            _.asServiceResponse
              .flatMap(
                _.toRight(ServiceError.notFoundEntityError(s"Document $docId couldn't be found"))
              )
          )
      }
      ...
      bytes ← EitherT {
        tmpBlobDao
          .loadBlob(name, approvedBy)
          .map(
            _.asServiceResponse
              .flatMap(
                _.toRight(
                  ServiceError.notFoundEntityError(s"Document blob '$name' couldn't be found")
                )
              )
          )
      }
  ...
  def rejectPendingDocument(docId: UUID, rejectedBy: String): F[ServiceResponse[Document]] = {
    (for {
      doc ← EitherT {
        dao
          .reject(docId, rejectedBy)
          .map(
            _.asServiceResponse
              .flatMap(
                _.toRight(ServiceError.notFoundEntityError(s"Document $docId couldn't be found"))
  ...
  def loadBlob(docId: UUID, blobId: String, userName: String): F[ServiceResponse[Array[Byte]]] = {
    for {
      doc ← blobDao.loadBlob(blobId, userName)
    } yield doc.asServiceResponse
      .flatMap(
        _.toRight(
          ServiceError.notFoundEntityError(s"Blob $blobId for doc $docId couldn't be found")
        )
      )
  }
...
object DocumentAlgebra {
  private def genBlobName(customerId: UUID, fileName: String): String =
    s"$customerId/${UUID.randomUUID()}_$fileName"
}
