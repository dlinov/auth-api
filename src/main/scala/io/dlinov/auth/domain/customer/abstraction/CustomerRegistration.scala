...
import io.dlinov.auth.domain.customer.model.CardApplicationAttributes.{
  CardApplication,
  CardPin,
  NewCardApplicationDetails
}
...
trait CustomerRegistration {
  ...
  def createDefaultBusinessUserAccount(user: RegisteredButNotActivatedBusinessUser)(
      implicit doneByBackOfficeUser: BackOfficeUser
  ): Future[ServiceResponse[Account]]

  def createCardApplication(
      user: RegisteredButNotActivatedBusinessUser,
      cardPin: Option[CardPin],
      nameOnCard: NameAttribute,
      cardDeliveryAddress: Address
  )(implicit doneByBackOfficeUser: BackOfficeUser): Future[ServiceResponse[CardApplication]]
