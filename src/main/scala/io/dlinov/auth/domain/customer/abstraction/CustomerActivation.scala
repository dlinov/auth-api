...
import io.dlinov.auth.domain.auth.entities.BackOfficeUser
import io.dlinov.auth.domain.customer.model.BusinessUsers.RegisteredButNotActivatedBusinessUser
import io.dlinov.auth.domain.customer.model.CustomerAttributes.{ActivationRequirement, CustomerStatus, LoginUsername}
...
trait CustomerActivation {
  ...
  def activateBusinessUserOnTheFly(
    ...
    requiredDocuments: Seq[ActivationRequirement])(implicit doneByBackOfficeUser: BackOfficeUser): Future[ServiceResponse[CustomerStatus]]

  def activateBusinessUser(user: RegisteredButNotActivatedBusinessUser)(implicit doneByBackOfficeUser: BackOfficeUser): Future[ServiceResponse[CustomerStatus]]
