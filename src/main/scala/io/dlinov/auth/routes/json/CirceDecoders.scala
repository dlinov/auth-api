...
object CirceDecoders extends CirceConfigProvider {
  ...
  implicit val buToReadDecoder: Decoder[BusinessUnitToRead] = deriveConfiguredDecoder[BusinessUnitToRead]
  implicit val buToCreateDecoder: Decoder[BusinessUnitToCreate] = deriveConfiguredDecoder[BusinessUnitToCreate]
    .ensure(_.name.nonEmpty, "name must not be empty")
  ...
  implicit val buToUpdateDecoder: Decoder[BusinessUnitToUpdate] = deriveConfiguredDecoder[BusinessUnitToUpdate]
    .ensure(_.name.nonEmpty, "name must not be empty")
    .ensure(_.name.length <= buNameMaxLength, s"name length must not exceed $buNameMaxLength characters")
    .ensure(bu ⇒ buNameRegex.findFirstMatchIn(bu.name).isDefined, s"name must follow pattern $buNameRegex")
  ...
  implicit val permissionToReadDecoder: Decoder[PermissionToRead] = deriveConfiguredDecoder[PermissionToRead]

  private val bouUserNameRegex: Regex = "^[A-Za-z0-9_\\.]{3,128}$".r
  private val bouNameRegex: Regex = "^[A-Za-z'\\.\\-\\s]{1,128}$".r
  implicit val bouToReadDecoder: Decoder[BackOfficeUserToRead] = deriveConfiguredDecoder[BackOfficeUserToRead]
  implicit val bouToCreateDecoder: Decoder[BackOfficeUserToCreate] = deriveConfiguredDecoder[BackOfficeUserToCreate]
  ...
  implicit val bouToUpdateDecoder: Decoder[BackOfficeUserToUpdate] = deriveConfiguredDecoder[BackOfficeUserToUpdate]
  ...
  implicit val pToCreateDecoder: Decoder[PermissionToCreate] = deriveConfiguredDecoder[PermissionToCreate]
  implicit val pToUpdateDecoder: Decoder[PermissionToUpdate] = deriveConfiguredDecoder[PermissionToUpdate]

  implicit val credentialsToReadDecoder: Decoder[CredentialsToRead] = deriveConfiguredDecoder[CredentialsToRead]
  ...
  implicit val credentialsToUpdateDecoder: Decoder[CredentialsToUpdate] = deriveConfiguredDecoder[CredentialsToUpdate]
    .ensure(_.user.nonEmpty, "user name must not be empty")
  ...
  implicit val loginStatusResponseDecoder: Decoder[LoginStatusResponse] = deriveConfiguredDecoder[LoginStatusResponse]
  ...
  implicit val passwordResetClaimDecoder: Decoder[PasswordResetClaim] = deriveConfiguredDecoder[PasswordResetClaim]
  ...
  implicit val documentToCreateDecoder: Decoder[DocumentToCreate] = deriveConfiguredDecoder[DocumentToCreate]
