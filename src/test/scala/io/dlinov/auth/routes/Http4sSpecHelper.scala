...
trait Http4sSpecHelper { self: MustMatchers ⇒
  ...
  def check[T](
    ...
    validateBody: T ⇒ Assertion)(implicit ev: EntityDecoder[IO, T]): Assertion = {
  ...
  def checkRawBody(
    actual: IO[Response[IO]],
    expectedStatus: Status,
    validateBody: String ⇒ Assertion): Assertion = {
    val actualResp = actual.unsafeRunSync
    actualResp.status mustBe expectedStatus
  ...
  def buildPostRequest[T](
    uri: String,
    entity: T)(implicit encoder: EntityEncoder[IO, T]): Request[IO] =
  ...
  def buildPostRequest[T](
    uri: String,
    entity: T,
    token: String)(implicit encoder: EntityEncoder[IO, T]): Request[IO] =
    buildPostRequest(uri, entity)
      .withHeaders(
        headers.`Content-Type`(MediaType.application.json),
        headers.Authorization(Credentials.Token(AuthScheme.Bearer, token)))

  def buildPutRequest[T](
    uri: String,
    entity: T)(implicit encoder: EntityEncoder[IO, T]): Request[IO] =
