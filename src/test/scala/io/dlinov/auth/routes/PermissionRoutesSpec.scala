...
class PermissionRoutesSpec extends Http4sSpec {
  ...
  "Permission routes" should {
    "grant new permissions" in {
      ...
      val resp1 = services.run(
        buildPostRequest[PermissionToCreate](
          uri = baseRoute,
          entity = pToCreate1,
    ...
    "find permissions by permission key params" in {
      ...
      val resp1 = services.run(buildGetRequest(uri = uri1, token = initialData.superAdminToken))
      ...
      val resp2 = services.run(buildGetRequest(uri = uri2, token = initialData.superAdminToken))
      ...
      val resp3 = services.run(buildGetRequest(uri = uri3, token = initialData.superAdminToken))
      check[PaginatedResult[PermissionToRead]](resp3, Status.Ok, permissions ⇒ {
        permissions.results.size mustBe 1
        permissions.total mustBe 1
      })
      val uri4  = uri3 + s"&offset=1"
      val resp4 = services.run(buildGetRequest(uri = uri4, token = initialData.superAdminToken))
